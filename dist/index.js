...
async function gql(endpoint, accessToken, query, variables) {
  ...
  if (json.errors?.length) {
    throw new Error(`HashLock API: ${json.errors.map((e) => e.message).join(", ")}`);
  }
  if (!json.data) {
...
// src/index.ts
var ENDPOINT = process.env.HASHLOCK_ENDPOINT || "http://142.93.106.129/graphql";
var ACCESS_TOKEN = process.env.HASHLOCK_ACCESS_TOKEN || "";
